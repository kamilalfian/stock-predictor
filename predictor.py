...
y_predicted=(y_predicted*(scaler.data_max_-scaler.data_min_)+scaler.data_min_).reshape(-1)
y_test=y_test*(scaler.data_max_-scaler.data_min_)+scaler.data_min_

# Calculate MAPE
def calculate_mape(y_true, y_pred):
    return np.mean(np.abs((y_true - y_pred[:-1]) / y_true)) * 100
...
y_test=pd.DataFrame(y_test,index=test.index,columns=['Original Price'])
y_predicted = pd.concat([pd.DataFrame(y_predicted[:-1], index=test.index, columns=['Predicted Price']),
                         pd.DataFrame(y_predicted[-1:], index=[test.index[-1] + pd.offsets.BDay(1)], columns=['Predicted Price'])])
